...
@implementation YLImageFrame
+ (instancetype)frameWithImage:(UIImage *)image {
    YLImageFrame *frame = [self new];
    frame.image = image;
    return frame;
}
- (id)copyWithZone:(NSZone *)zone {
    YLImageFrame *frame = [self.class new];
    frame.index = _index;
    frame.width = _width;
...
@interface _YYImageDecoderFrame : YLImageFrame
...
+ (instancetype)decoderWithData:(NSData *)data scale:(CGFloat)scale {
    if (!data) return nil;
    YLImageDecoder *decoder = [[YLImageDecoder alloc] initWithScale:scale];
...
- (YLImageFrame *)frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
    YLImageFrame *result = nil;
    pthread_mutex_lock(&_lock);
...
- (YLImageFrame *)_frameAtIndex:(NSUInteger)index decodeForDisplay:(BOOL)decodeForDisplay {
...
+ (NSData *)encodeImage:(UIImage *)image type:(YYImageType)type quality:(CGFloat)quality {
    YLImageEncoder *encoder = [[YLImageEncoder alloc] initWithType:type];
    encoder.quality = quality;
    [encoder addImage:image duration:0];
    return [encoder encode];
}

+ (NSData *)encodeImageWithDecoder:(YLImageDecoder *)decoder type:(YYImageType)type quality:(CGFloat)quality {
...
@implementation UIImage (YLImageCoder)
...
- (NSData *)_imageDataRepresentationForSystem:(BOOL)forSystem {
    NSData *data = nil;
    if ([self isKindOfClass:[YLImage class]]) {
        YLImage *image = (id)self;
