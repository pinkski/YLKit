...
#import "YLDiskCache.h"
#import "UIImage+YLAdd.h"
//#import "NSObject+YLAdd.h"
#import "YLImage.h"
...
- (UIImage *)imageFromData:(NSData *)data {
    NSData *scaleData = [YLDiskCache getExtendedDataFromObject:data];
    CGFloat scale = 0;
    if (scaleData) {
    ...
    if (_allowAnimatedImage) {
        image = [[YLImage alloc] initWithData:data scale:scale];
        if (_decodeForDisplay) image = [image imageByDecoded];
    } else {
        YLImageDecoder *decoder = [YLImageDecoder decoderWithData:data scale:scale];
...
- (instancetype)initWithPath:(NSString *)path {
    YLMemoryCache *memoryCache = [YLMemoryCache new];
    ...
    YLDiskCache *diskCache = [[YLDiskCache alloc] initWithPath:path];
...
- (void)setImage:(UIImage *)image imageData:(NSData *)imageData forKey:(NSString *)key withType:(YYImageCacheType)type {
    ...
    if (type & YYImageCacheTypeDisk) { // add to disk cache
        if (imageData) {
            if (image) {
                [YLDiskCache setExtendedData:[NSKeyedArchiver archivedDataWithRootObject:@(image.scale)] toObject:imageData];
            }
            ...
            dispatch_async(YYImageCacheIOQueue(), ^{
                ...
                [YLDiskCache setExtendedData:[NSKeyedArchiver archivedDataWithRootObject:@(image.scale)] toObject:data];
