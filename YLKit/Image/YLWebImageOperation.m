...
#if __has_include("YLDispatchQueuePool.h")
...
@interface YLWebImageOperation() <NSURLConnectionDelegate>
...
@property (nonatomic, strong) YLImageDecoder *progressiveDecoder;
...
@implementation YLWebImageOperation
...
- (void)_startRequest:(id)object {
    ...
    @autoreleasepool {
        ...
        [_lock lock];
        if (![self isCancelled]) {
            _connection = [[NSURLConnection alloc] initWithRequest:_request delegate:[YLWeakProxy proxyWithTarget:self]];
            if (![_request.URL isFileURL] && (_options & YYWebImageOptionShowNetworkActivity)) {
        ...
        if (![self isCancelled]) {
            if (_cache) {
                if (image || (_options & YYWebImageOptionRefreshImageCache)) {
                    NSData *data = _data;
                    dispatch_async([YLWebImageOperation _imageQueue], ^{
...
- (void)connection:(NSURLConnection *)connection didReceiveData:(NSData *)data {
    @autoreleasepool {
        ...
        if (!_progressiveDecoder) {
            _progressiveDecoder = [[YLImageDecoder alloc] initWithScale:[UIScreen mainScreen].scale];
        }
        ...
        if (!progressiveBlur) {
            YLImageFrame *frame = [_progressiveDecoder frameAtIndex:0 decodeForDisplay:YES];
            if (frame.image) {
                [_lock lock];
                if (![self isCancelled]) {
    ...
    @autoreleasepool {
        ...
        if (![self isCancelled]) {
            ...
            dispatch_async([self.class _imageQueue], ^{
                ...
                if (allowAnimation) {
                    image = [[YLImage alloc] initWithData:self.data scale:[UIScreen mainScreen].scale];
                    if (shouldDecode) image = [image imageByDecoded];
                    if ([((YLImage *)image) animatedImageFrameCount] > 1) {
                        hasAnimation = YES;
                    }
                } else {
                    YLImageDecoder *decoder = [YLImageDecoder decoderWithData:self.data scale:[UIScreen mainScreen].scale];
