...
#import "NSObject+YLModel.h"
#import "YLClassInfo.h"
...
/**
 *  property info in a obj
 */
@interface _YLModelPropertyMeta : NSObject {
    @package
    NSString *_name;
    ...
    YLClassPropertyInfo *_info;
    _YLModelPropertyMeta *_next;
}
@end

@implementation _YLModelPropertyMeta
+ (instancetype)metaWithClassInfo:(YLClassInfo *)classInfo propertyInfo:(YLClassPropertyInfo *)propertyInfo generic:(Class)generic {
    ...
    _YLModelPropertyMeta *meta = [self new];
...
@interface _YLModelMeta : NSObject {
    @package
    YLClassInfo *_classInfo;
...
@implementation _YLModelMeta
- (instancetype)initWithClass:(Class)cls {
    YLClassInfo *classInfo = [YLClassInfo classInfoWithClass:cls];
    ...
    if ([cls respondsToSelector:@selector(modelPropertyBlacklist)]) {
        NSArray *properties = [(id<YLModel>)cls modelPropertyBlacklist];
        if (properties) {
    ...
    if ([cls respondsToSelector:@selector(modelPropertyWhitelist)]) {
        NSArray *properties = [(id<YLModel>)cls modelPropertyWhitelist];
        if (properties) {
    ...
    if ([cls respondsToSelector:@selector(modelContainerPropertyGenericClass)]) {
        genericMapper = [(id<YLModel>)cls modelContainerPropertyGenericClass];
        if (genericMapper) {
    ...
    YLClassInfo *curClassInfo = classInfo;
    while (curClassInfo && curClassInfo.superCls != nil) { // recursive parse super class, but ignore root class (NSObject/NSProxy)
        for (YLClassPropertyInfo *propertyInfo in curClassInfo.propertyInfos.allValues) {
            if (!propertyInfo.name) continue;
            if (blacklist && [blacklist containsObject:propertyInfo.name]) continue;
            if (whitelist && ![whitelist containsObject:propertyInfo.name]) continue;
            _YLModelPropertyMeta *meta = [_YLModelPropertyMeta metaWithClassInfo:classInfo
    ...
    if ([cls respondsToSelector:@selector(modelCustomPropertyMapper)]) {
        NSDictionary *customMapper = [(id <YLModel>)cls modelCustomPropertyMapper];
        [customMapper enumerateKeysAndObjectsUsingBlock:^(NSString *propertyName, NSString *mappedToKey, BOOL *stop) {
            _YLModelPropertyMeta *propertyMeta = allPropertyMetas[propertyName];
    ...
    [allPropertyMetas enumerateKeysAndObjectsUsingBlock:^(NSString *name, _YLModelPropertyMeta *propertyMeta, BOOL *stop) {
...
+ (instancetype)metaWithClass:(Class)cls {
    ...
    _YLModelMeta *meta = CFDictionaryGetValue(cache, (__bridge const void *)(cls));
    dispatch_semaphore_signal(lock);
    if (!meta || meta->_classInfo.needUpdate) {
        meta = [[_YLModelMeta alloc] initWithClass:cls];
        if (meta) {
            dispatch_semaphore_wait(lock, DISPATCH_TIME_FOREVER);
...
static force_inline NSNumber *ModelCreateNumberFromProperty(__unsafe_unretained id model,
                                                            __unsafe_unretained _YLModelPropertyMeta *meta) {
...
/// set value to model with property meta
...
/// apply function for dictionary , to set key value to model
static void ModelSetWithDictionaryFunction(const void *_key, const void *_value, void *_context) {
    ModelSetContext *context = _context;
    __unsafe_unretained _YLModelMeta *meta = (__bridge _YLModelMeta *)(context->modelMeta);
    __unsafe_unretained _YLModelPropertyMeta *propertyMeta = [meta->_mapper objectForKey:(__bridge id)(_key)];
...
static void ModelSetWithPropertyMetaArrayFunction(const void *_propertyMeta, void *_context) {
    ...
    __unsafe_unretained _YLModelPropertyMeta *propertyMeta = (__bridge _YLModelPropertyMeta *)(_propertyMeta);
...
static id ModelToJSONObjectRecursive(NSObject *model) {
    ...
    _YLModelMeta *modelMeta = [_YLModelMeta metaWithClass:[model class]];
    if (!modelMeta || modelMeta->_keyMappedCount == 0) return nil;
    NSMutableDictionary *result = [[NSMutableDictionary alloc] initWithCapacity:64];
    __unsafe_unretained NSMutableDictionary *dic = result; // avoid retain and release in block
    [modelMeta->_mapper enumerateKeysAndObjectsUsingBlock:^(NSString *propertyMappedKey, _YLModelPropertyMeta *propertyMeta, BOOL *stop) {
    ...
    if (modelMeta->_hasCustomTransformToDictionary) {
        BOOL suc = [((id<YLModel>)model) modelCustomTransformToDictionary:dic];
        if (!suc) return nil;
    }
    return result;
}
    ...
    switch (modelMeta->_nsType) {
        ...
        default: {
            ...
            NSArray *properties = [modelMeta->_allPropertyMetas
                                   sortedArrayUsingComparator:^NSComparisonResult(_YLModelPropertyMeta *p1, _YLModelPropertyMeta *p2) {
                                       return [p1->_name compare:p2->_name];
                                   }];
            
            [desc appendFormat:@" {\n"];
            for (NSUInteger i = 0, max = properties.count; i < max; i++) {
                _YLModelPropertyMeta *property = properties[i];
...
@implementation NSObject (YLModel)
...
+ (instancetype)modelWithDictionary:(NSDictionary *)dictionary {
    ...
    _YLModelMeta *modelMeta = [_YLModelMeta metaWithClass:cls];
...
- (BOOL)modelSetWithDictionary:(NSDictionary *)dic {
    ...
    _YLModelMeta *modelMeta = [_YLModelMeta metaWithClass:object_getClass(self)];
    if (modelMeta->_keyMappedCount == 0) return NO;
    
    if (modelMeta->_hasCustomWillTransformFromDictionary) {
        dic = [((id<YLModel>)self) modelCustomWillTransformFromDictionary:dic];
        if (![dic isKindOfClass:[NSDictionary class]]) return NO;
    }
    ...
    if (modelMeta->_hasCustomTransformFromDictionary) {
        return [((id<YLModel>)self) modelCustomTransformFromDictionary:dic];
    }
    return YES;
}
...
- (NSData *)modelToJSONData {
    id jsonObject = [self modelToJSONObjct];
...
- (id)modelCopy {
    ...
    _YLModelMeta *modelMeta = [_YLModelMeta metaWithClass:self.class];
    if (modelMeta->_nsType) return [self copy];
    
    NSObject *one = [self.class new];
    for (_YLModelPropertyMeta *propertyMeta in modelMeta->_allPropertyMetas) {
