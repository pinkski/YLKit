    ...
    while (!finish) {
        if (pthread_mutex_trylock(&_lock) == 0) {
            if (_lru->_totalCost > costLimit) {
                _YLLinkedMapNode *node = [_lru removeTailNode];
...
- (instancetype)init {
    ...
    _lru = [_YLLinkedMap new];
    _queue = dispatch_queue_create("com.yolynn.cache.memory", DISPATCH_QUEUE_SERIAL);
