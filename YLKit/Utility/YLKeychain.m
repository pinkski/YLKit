...
- (id)copyWithZone:(NSZone *)zone {
    YLKeychainItem *item = [YLKeychainItem new];
...
+ (NSString *)getPasswordForService:(NSString *)serviceName
    ...
    if (!serviceName || !account) {
        if (error) *error = [YLKeychain errorWithCode:errSecParam];
        return nil;
    }
    ...
    YLKeychainItem *result = [self selectOneItem:item error:error];
...
+ (BOOL)setPassword:(NSString *)password
    ...
    item.service = serviceName;
    item.account = account;
    YLKeychainItem *result = [self selectOneItem:item error:NULL];
    if (result) {
...
+ (BOOL)insertItem:(YLKeychainItem *)item error:(NSError **)error {
    ...
    if (status != errSecSuccess) {
        if (error) *error = [YLKeychain errorWithCode:status];
        return NO;
    }
...
+ (BOOL)insertItem:(YLKeychainItem *)item {
    return [self insertItem:item error:NULL];
}

+ (BOOL)updateItem:(YLKeychainItem *)item error:(NSError **)error {
    if (!item.service || !item.account || !item.passwordData) {
        if (error) *error = [YLKeychain errorWithCode:errSecParam];
        return NO;
    }
...
+ (BOOL)updateItem:(YLKeychainItem *)item {
    return [self updateItem:item error:NULL];
}

+ (BOOL)deleteItem:(YLKeychainItem *)item error:(NSError **)error {
...
+ (BOOL)deleteItem:(YLKeychainItem *)item {
    return [self deleteItem:item error:NULL];
}

+ (YLKeychainItem *)selectOneItem:(YLKeychainItem *)item error:(NSError **)error {
    if (!item.service || !item.account) {
        if (error) *error = [YLKeychain errorWithCode:errSecParam];
        return nil;
    }
    ...
    return [[YLKeychainItem alloc] initWithDic:dic];
}

+ (YLKeychainItem *)selectOneItem:(YLKeychainItem *)item {
    return [self selectOneItem:item error:NULL];
}

+ (NSArray *)selectItems:(YLKeychainItem *)item error:(NSError **)error {
    NSMutableDictionary *query = [item dic];
    ...
    if (CFGetTypeID(result) == CFDictionaryGetTypeID()) {
        dic = (__bridge NSDictionary *)(result);
        YLKeychainItem *item = [[YLKeychainItem alloc] initWithDic:dic];
...
+ (NSArray *)selectItems:(YLKeychainItem *)item {
