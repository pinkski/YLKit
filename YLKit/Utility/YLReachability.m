...
static void YYReachabilityCallback(SCNetworkReachabilityRef target, SCNetworkReachabilityFlags flags, void *info) {
    YLReachability *self = ((__bridge YLReachability *)info);
    if (self.notifyBlock) {
...
+ (instancetype)reachabilityForLocalWifi {
    ...
    YLReachability *one = [self reachabilityWithAddress:(const struct sockaddr *)&localWifiAddress];
    one.allowWWAN = NO;
    return one;
}
...
- (void)setNotifyBlock:(void (^)(YLReachability *reachability))notifyBlock {
